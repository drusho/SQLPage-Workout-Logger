...
**Description:** Processes a multi-set workout form submission from index.sql. It first creates a single parent entry in the `WorkoutLog` table, then saves each submitted set individually into the `WorkoutSetLog` table, and finally updates the user's progression record to advance them to the next step.

**Requires:**
- The `WorkoutLog` and `WorkoutSetLog` tables for saving workout data.
- The `UserExerciseProgression` table to track and update user progress.

**Parameters:**
- template_id [form] The ID of the parent workout template.
- exercise_id [form] The ID of the exercise being logged.
- num_sets [form] The total number of sets submitted from the form.
- reps_1, weight_1, ... [form] The dynamically named form fields for each set's reps and weight.
- rpe_recorded [form] The overall Rate of Perceived Exertion for the workout session.
- notes_recorded [form, optional] User-provided notes for the entire workout.

**Returns:**
- A `redirect` component that sends the user back to the main workout page, keeping the current workout template selected.

**See Also:**
- `index.sql` - The page that contains the form that submits to this action.
- `views/view_full_workout_history.sql` - A page that displays data saved by this script.

**Notes:**
- This script uses a series of conditional INSERTs to save each set. This is a reliable pattern in SQLPage for handling a variable number of form fields.
...
**Description:** The main application dashboard for logging a workout. This is a multi-step, self-reloading form that uses the new database views to display progression targets and a dynamic, multi-set form for logging.

**Requires:**
- `layouts/layout_main.sql` for the page shell.
- `views/UserExerciseProgressionTargets` VIEW for target calculations.
- `views/WorkoutTemplateDetails` VIEW for listing exercises.

**Parameters:**
- template_id [url, optional] The ID of the selected workout template.
- selected_exercise_id [url, optional] The ID of the chosen exercise to log.

**Returns:**
- A full UI page that progressively reveals more components as the user makes selections.

**See Also:**
- `actions/action_save_workout.sql` - The script that the new multi-set form will submit to.

**Notes:**
- This page uses a progressive disclosure UI. The workout logging form (Step 6) is hidden until an exercise is selected in Step 5.
- The target data query is now robust, providing default values if no pre-existing progression is found for a user on a given exercise. and `WorkoutSetLog` tables.
